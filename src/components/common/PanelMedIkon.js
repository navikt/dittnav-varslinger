...
const PanelMedIkon = (props) => {
  ...
  const utvidTekstLenke = (
    <span>
      {linebreak(isOpen)}
      <Lenke
        className={`utvid-tekst-lenke__${isOpen ? "skjul" : "aapne"}`}
        href="#"
        onClick={(event) => utvidTekst(event)}
      >
  ...
  return (
    <Panel className={props.className} border>
      <div className={`${props.className}__ikon`}>{props.children}</div>
      <div className={`${props.className}__tekst`}>
        {props.erInformasjonsmelding ? (
          props.overskrift
        ) : (
          <Normaltekst>
            <span>
        ...
        {props.ingress ? <Normaltekst>{props.ingress}</Normaltekst> : null}
        {props.etikett ? <Undertekst className={`${props.className}__etikett`}>{props.etikett}</Undertekst> : ""}
      </div>
      <>
        {props.knapp ? (
...
PanelMedIkon.propTypes = {
  onClick: func,
  className: string,
  overskrift: oneOfType([string, shape({ root: any })]).isRequired,
