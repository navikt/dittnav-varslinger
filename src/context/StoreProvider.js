...
const StoreProvider = ({ children }) => {
  ...
  const addBeskjeder = (nyeBeskjeder) =>
    dispatch({
      type: "ADD_BESKJEDER",
      payload: nyeBeskjeder,
    });

  const addInaktiveBeskjeder = (nyeBeskjeder) =>
    dispatch({
      type: "ADD_INAKTIVE_BESKJEDER",
      payload: nyeBeskjeder,
    });

  const removeBeskjed = (beskjed) =>
    dispatch({
      type: "REMOVE_BESKJED",
      payload: beskjed,
    });

  const addInaktivBeskjed = (beskjed) =>
    dispatch({
      type: "ADD_INAKTIV_BESKJED",
      payload: beskjed,
    });

  const toggleInnloggingsModal = () =>
    dispatch({
