...
const FormHendelser = ({ tekst, lenke, valg, eksternVarsling, setTekst, setLenke, setOppgaver, setInnbokser }) => {
  ...
  const getBrukernotifikasjoner = () => {
    fetchBeskjeder().then(([r]) => {
      addBeskjeder(r);
    });

    fetchOppgaver().then(([r]) => {
      setOppgaver(r);
    });

    fetchInnbokser().then(([r]) => {
      setInnbokser(r);
    });
  };
  ...
  const convertStringToBoolean = (value) => value === "true";
  ...
  const handleSubmit = (event) => {
    ...
    postHendelse(
      `produce/${valg}`,
      shouldSendEksternVarsling ? postContentForBeskjedAndOppgave : postContentForInnboks
    );
    clearInput();
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      />
      <Input label="Skriv inn ny grupperingsid:" value={grupperingsid} onChange={(e) => handleGrupperingsid(e)} />
